...
smallStep (Store expr, acc)
  | isValue expr = Just (Const 0, expr) -- Update the accumulator with the value and return `Const 0`
  | isThrow expr = Just (expr, acc)    -- Bubble the exception without updating the accumulator
